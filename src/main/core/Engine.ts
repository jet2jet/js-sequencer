	...
	if (!disableHold) {
		for (; fromIndex < arr.length; ++fromIndex) {
			...
			if (!isHolding) {
				...
				if (posTo.numerator * n.notePosDenominator <= n.notePosNumerator * posTo.denominator) {
					break;
				}
			}
...
function calculatePositionImpl(
	...
	for (let i = 0; i < notesAndControls.length; ++i) {
		...
		if (isPosSeconds) {
			...
			if (tFrom === null && note.notePosNumerator * retFrom!.denominator >= retFrom!.numerator * note.notePosDenominator) {
				if (returnGreaterOrEqual) {
					posNum = note.notePosNumerator;
					posDen = note.notePosDenominator;
				}
					...
					switch (note.value1) {
						...
						case 0x62: // NRPN LSB
							if (note.value2 >= 0 && note.value2 <= 0x7F) {
								...
								if (ch.nrpnMsb !== null) {
									ch.rpnValue = 0x4000 + ch.nrpnMsb * 0x80 + ch.nrpnLsb;
								}
							}
							break;
						case 0x63: // NRPN MSB
						...
						case 0x64: // RPN LSB
							...
							if (note.value2 >= 0 && note.value2 <= 0x7F) {
								...
								if (ch.rpnMsb !== null) {
									...
									if (ch.rpnValue === 0x3FFF) // RPN NULL
										ch.rpnValue = null;
								}
							}
							break;
...
export function calculatePositionFromSeconds(
	...
	if (typeof (timeSecondsFrom) !== typeof (0) || typeof (timeSecondsTo) !== typeof (0) ||
...
// return: deltaTime
function _getDeltaTime(posNum: number, posDen: number, division: number): number {
	return posNum * division * 4 / posDen;
}

function _addNoteFromDeltaTime(
	notesArray: NoteObject[], noteValue: number, deltaTime: number,
	noteLengthTime: number, division: number, velocity: number, channel: number
) {
	...
	const n = new NoteObject(posNum, posDen, noteLengthNum, noteLengthDen, noteValue, channel);
	n.velocity = velocity;
	notesArray.push(n);
...
function _startLoadSMFData(ctx: ILoadSMFContext, smfBuffer: ArrayBuffer, offset: number): ILoadSMFContext | null {
	if (smfBuffer.byteLength - offset < 8)
		return null;
	const dv = new DataView(smfBuffer, offset);
	// check data header
	if (dv.getUint32(0, true) !== 0x6468544D) // "MThd"
		return null;
	...
	if (format >= 2)  // only supports format = 0 or format = 1
		return null;
...
function loadFromSMFTrack(
	trackBuffer: ArrayBuffer, offset: number, division: number
	...
	// check header format
	if (dv.getUint32(0, true) !== 0x6B72544D) { // "MTrk"
	...
	while (true) {
		...
		// read delta-time
		if (off >= len) {
			throw new Error(`Not enough data for time (offset = ${off.toString()})`);
		}
		let b = dv.getUint8(off++);
		while (b >= 0x80) { // variable-length quantity
			if (off >= len) {
				throw new Error(`Not enough data for variable number (offset = ${off.toString()})`);
			}
			tm += b & 0x7F;
			tm <<= 7;
			b = dv.getUint8(off++);
		...
		// read status
		if (off >= len) {
			throw new Error(`Not enough data for status (offset = ${off.toString()})`);
		}
		b = dv.getUint8(off++);
		if (b < 0x80 && !bmsg) {
			throw new Error(`Time data without status data: 0x${b.toString(16)}`);
		}
		let msg;
		if (b < 0x80) {
			msg = bmsg;
			...
			bmsg = msg = b;
			if (off >= len) {
				throw new Error(`Not enough second data (status omitted): status = 0x${msg.toString(16)} (offset = ${off.toString()})`);
			}
			b = dv.getUint8(off++);
		}
		...
		if (msg >= 0xF0) {
			if (msg === 0xFF) {
				...
				if (off + smsglen > len) {
					throw new Error(`Not enough second data: status = 0x${msg.toString(16)} (offset = ${off.toString()})`);
				}
				if (b === 0x2F) { // end of track
					addToControlArray(retControls, new EOTObject(p.numerator, p.denominator));
					break;
				}
				switch (b) {
					case 0x51: // tempo
						{
							if (smsglen !== 3) {
								throw new Error(`Invalid length for FF 51: ${smsglen.toString()}`);
							}
							// read four-bytes from 'off - 1' in big-endian and chop the highest byte
							const tempo = dv.getUint32(off - 1, false) & 0x00FFFFFF;
							addToControlArray(retMControls, new TempoControl(p.numerator, p.denominator, tempo));
						}
						break;
					case 0x58: // time signature
						{
							if (smsglen !== 4) {
								throw new Error(`Invalid length for FF 58: ${smsglen.toString()}`);
							}
							...
							addToControlArray(retMControls, new TimeSignatureControl(p.numerator, p.denominator, beats, beatsDen, clocks, num32nd));
						}
						break;
					case 0x59: // signature
						{
							if (smsglen !== 2) {
								throw new Error(`Invalid length for FF 59: ${smsglen.toString()}`);
							}
							const sf = dv.getInt8(off);
							const ism = dv.getUint8(off + 1);
							addToControlArray(retMControls, new KeySignatureControl(p.numerator, p.denominator, sf, !!ism));
						}
						break;
					default:
						addToControlArray(retControls,
							new SysMsgControl(p.numerator, p.denominator, b, trackBuffer, offset + off, smsglen)
						);
						break;
				}
				off += smsglen;
			...
			switch (msg & 0xF0) {
				...
				case 0x90: // Note on (with 2-bytes)
					{
						...
						if (vel >= 0x80) {
							throw new Error(`Invalid velocity '${vel.toString()}': status = 0x${msg.toString(16)}`);
						}
						if ((msg & 0xF0) === 0x80)
							vel = 0;
						if (!vel && curNotes[ch][b].velocity) {
							_addNoteFromDeltaTime(retNotes, b, curNotes[ch][b].time, del - curNotes[ch][b].time,
								division, curNotes[ch][b].velocity, ch);
				...
				case 0xA0: // Aftertouch (with 2-bytes)
					{
						...
						addToControlArray(retControls,
							new AftertouchControl(p.numerator, p.denominator, b, msg & 0x0F, val)
						);
					}
					break;
				case 0xB0: // Controller (with 2-bytes)
					{
						...
						const val = dv.getUint8(off++);
						addToControlArray(retControls,
							new ControllerControl(p.numerator, p.denominator, msg & 0x0F, b, val)
						);
					}
					break;
				case 0xC0: // Program Change (with 1-byte)
					{
						addToControlArray(retControls,
							new ProgramChangeControl(p.numerator, p.denominator, msg & 0x0F, b)
						);
					}
					break;
				case 0xD0: // Channel Pressure (with 1-byte)
					{
						addToControlArray(retControls,
							new PressureControl(p.numerator, p.denominator, msg & 0x0F, b)
						);
					}
					break;
				case 0xE0: // Pitch Wheel (with 2-bytes)
					{
						...
						const val = dv.getUint8(off++);
						addToControlArray(retControls,
							new PitchWheelControl(p.numerator, p.denominator, msg & 0x0F, b | (val << 7))
						);
					}
					break;
			}
...
function startLoadFromSMFFileImpl(fileObject: Blob, callback?: (ctx: ILoadSMFContext | null) => void) {
...
function copyUint8ArrayToDataView(dvOut: DataView, offsetOut: number, arr: Uint8Array) {
	if (arr.set) {
		new Uint8Array(dvOut.buffer, dvOut.byteOffset + offsetOut, arr.byteLength).set(
			arr
		);
...
function divideNoteObjects(notes: NoteObject[]) {
	...
	notes.forEach((n) => {
		...
		arr[j++] = new DummyNoteStopObject(
			n.notePosNumerator * n.noteLengthDenominator + n.noteLengthNumerator * n.notePosDenominator,
...
function calcTrackLength(notesAndControls: ISequencerObject[], division: number): number {
	...
	notesAndControls.forEach((o) => {
		...
		let dt = 0;
		if (beforePosNum * o.notePosDenominator !== o.notePosNumerator * beforePosDen) {
			dt = _getDeltaTime(
				o.notePosNumerator * beforePosDen - beforePosNum * o.notePosDenominator,
		...
		if (o instanceof NoteObject ||
			o instanceof DummyNoteStopObject) {
			if (before != null && (
			...
			if (before != null && before instanceof AftertouchControl &&
				before.channel === o.channel)
				len = 2;
			else
				len = 3;
		} else if (o instanceof ControllerControl) {
			if (before != null && before instanceof ControllerControl &&
				before.channel === o.channel)
				len = 2;
			else
				len = 3;
		} else if (o instanceof ProgramChangeControl) {
			if (before != null && before instanceof ProgramChangeControl &&
				before.channel === o.channel)
				len = 1;
			else
				len = 2;
		} else if (o instanceof PressureControl) {
			if (before != null && before instanceof PressureControl &&
				before.channel === o.channel)
				len = 1;
			else
				len = 2;
		} else if (o instanceof PitchWheelControl) {
			if (before != null && before instanceof PitchWheelControl &&
				before.channel === o.channel)
				len = 2;
			else
				len = 3;
		...
		if (o instanceof NoteObject ||
			...
			if (o instanceof SysExControl) {
				...
				if (o instanceof TempoControl) {
					...
					let v = 0;
					let x = 1;
					while (x < o.beatsDenominator) { ++v; x <<= 1; }
 ...
 */
function createMIDIData(parts: Part[], mcontrols: ControlObject[], div: number): ArrayBuffer {
	...
	parts.forEach((p, index) => {
		const d = ([] as ISequencerObject[]).concat(divideNoteObjects(p.notes)).concat(p.controls);
...
export default class Engine {
	...
	public smfDivision: number = 0x120;
	...
	public getMeasureFromPosition(numerator: number, denominator: number): number;
	public getMeasureFromPosition(pos: PositionObject): number;

	public getMeasureFromPosition(numerator: number | PositionObject, denominator?: number): number {
		...
		let iPos = 0;
		while (true) {
			if (ctrls && iCPos < ctrls.length && nextPosNum >= 0 &&
			...
			if (bc.posNumerator * denominator! >= numerator * bc.posDenominator)
				return measure;
		}
		// unreachable
	}
	...
	public getPositionFromMeasure(measure: number): IPositionObject {
		...
		let iPos = 0;
		while (true) {
			if (ctrls && iCPos < ctrls.length &&
				nextPosNum >= 0 && bc.posNumerator * nextPosDen >= nextPosNum * bc.posDenominator) {
				while (iCPos < ctrls.length) {
					const c = ctrls[iCPos];
					if (c instanceof TimeSignatureControl) {
						if (bc.posNumerator * c.notePosDenominator >= c.notePosNumerator * bc.posDenominator) {
			...
			if (iPos === 0) {
				measure--;
				if (measure < 0)
					return new PositionObject(bc.posNumerator, bc.posDenominator);
			}
			if (++iPos === bc.beatsNumerator) {
				iPos = 0;
			}
	...
	public addEventHandler<T extends keyof EngineEventObjectMap>(
		name: T, fn: (e: EngineEventObjectMap[T]) => void
	): void;
	public addEventHandler(name: string, fn: (e: EventObjectBase) => void) {
		let arr: any[] | null = null;
		switch (name.toLowerCase()) {
			case 'fileloaded': arr = this._evtFileLoaded; break;
		}
		if (!arr) return;
		arr.push(fn);
	}
	...
	public calculateSeconds(posFrom: IPositionObject, posTo: IPositionObject, disableHold?: boolean): {
		timeFrom: TimeValue,
		timeTo: TimeValue,
		timeStartOffset: TimeValue,
		duration: TimeValue
	...
	public continueLoadSMFData(ctx: ILoadSMFContext): boolean {
		...
		for (let track = 0; track < ctx.trackCount; ++track) {
			// check data format
			if (lenRemain < 8) {
				ctx.error = new Error('Not enough track header data: ' + track.toString());
				return false;
			}
			if (dv.getUint32(off, true) !== 0x6B72544D) {// "MTrk"
			...
			lenRemain -= 8;
			if (lenRemain < len) {
				ctx.error = new Error('Not enough track data: ' + track.toString());
				return false;
			}
			try {
				const ret = loadFromSMFTrack(ctx.smfBuffer, off + ctx.startOffset + 14, ctx.division);
	...
	public getErrorFromLoadSMFData(ctx: ILoadSMFContext | null): any {
		return ctx && ctx.smfBuffer !== null ? ctx.error : new Error('Invalid context');
	}
	...
	private loadContextCallbackImpl(ctx: ILoadSMFContext | null, callback: (err?: any) => void) {
		if (!ctx || ctx.error) {
	...
	public loadSMFData(smfBuffer: ArrayBuffer, offset: number, callback: (err?: any) => void) {
		return this.loadContextCallbackImpl(this.startLoadSMFData(smfBuffer, offset), callback);
	}

	public loadSMFDataPromise(smfBuffer: ArrayBuffer, offset: number) {
		return this.loadContextAsyncImpl(this.startLoadSMFData(smfBuffer, offset));
	}

	public loadFromFile(fileElemId: string | HTMLInputElement, callback?: (error?: any) => void) {
		const f: HTMLInputElement = (fileElemId && (fileElemId as HTMLInputElement).files) ? (fileElemId as HTMLInputElement) : (document.getElementById(fileElemId as string) as HTMLInputElement);
	...
	public exportSMFToArrayBuffer(): ArrayBuffer {
		return createMIDIData(this.parts, this.masterControls, this.smfDivision);
	}
	...
	public loadFromJSON(text: string): boolean {
		...
		for (let i = 0; i < this.masterControls.length; ++i) {
			this.masterControls[i] = getControlFromJSONObject(obj.masterControls[i]);
		}
