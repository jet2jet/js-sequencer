...
export function createWeakMap<K extends object, V>(): MyWeakMap<K, V> {
	...
	const o: MyPolyfillWeakMap = { _hash: '_my_weakmap_' + (++_myWeakMapId).toString() + '_' + Math.floor(Math.random() * 0x7FFFFFFF).toString(16).toLowerCase() };
	return o;
}

export function setWeakMap<K extends object, V>(map: MyWeakMap<K, V>, key: K, value: V): MyWeakMap<K, V> {
	if (_isWeakMapSupported) {
		return (map as MyWeakMapBase<K, V>).set(key, value);
	}
	if (!isMyWeakMap(map))
		throw new TypeError('map is not a weak map');
...
export function getWeakMap<K extends object, V>(map: MyWeakMap<K, V>, key: K): V | undefined {
	if (_isWeakMapSupported) {
...
export function hasWeakMapKey<K extends object, V>(map: MyWeakMap<K, V>, key: K): boolean {
	if (_isWeakMapSupported) {
	...
	if (key.hasOwnProperty)
		return key.hasOwnProperty(map._hash);
...
export function deleteWeakMapKey<K extends object, V>(map: MyWeakMap<K, V>, key: K): boolean {
	if (_isWeakMapSupported) {
